...
async def doc(bot,update):
     ...
     try:
     		path = await bot.download_media(message = file, progress=progress_for_pyrogram,progress_args=( "Uploading...",  ms, c_time   ))
     ...
     if value < file.file_size:
         ...
         try:
     		...
     		c_time = time.time()
     		try:
     			await bot.send_document(update.from_user.id,document = file_path,thumb=ph_path,caption = caption,progress=progress_for_pyrogram,progress_args=( "Downloading...",  ms, c_time   ))			
     			await ms.delete()
     			os.remove(file_path)
     		except Exception as e:
     			neg_used = used - int(file.file_size)
